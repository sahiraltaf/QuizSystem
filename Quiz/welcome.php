        ...
        <div class="row">
            <div class="col-md-12">
                ...
                foreach ($data as $item) {
                    ...
                    $types = mysqli_real_escape_string($con, $item['types']);
                    $difficulty = mysqli_real_escape_string($con, $item['difficulty']);
                    $question = mysqli_real_escape_string($con, $item['question']);
                    $correct_answer = mysqli_real_escape_string($con, $item['correct_answer']);
                
                    // create a SQL query to insert the values into the table
                    $query = "INSERT INTO quizjsondata (category, types, difficulty, question, correct_answer) 
                              VALUES ('$category', '$types', '$difficulty', '$question', '$correct_answer')";
                
                    // execute the query
                    mysqli_query($con, $query);
                
                    // loop through the incorrect answers and insert them into another table
                    foreach ($item['incorrect_answers'] as $incorrect_answer) {
                        $incorrect_answer = mysqli_real_escape_string($con, $incorrect_answer);
                        $query = "INSERT INTO quizjsondata (incorrect_answers) 
